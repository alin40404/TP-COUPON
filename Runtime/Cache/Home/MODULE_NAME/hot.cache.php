<? if (!class_exists('template')) die('Access Denied');$template->getInstance()->check('hot.html', 'ffc07e524744c16de6ed5a8b385c0be3', 1359708052);?>
            ...
            <div class="main_l">
                <div id="main_l_l" class="clear">
                    ...
                    <dl class="filter clear">
                        ...
                        <dd <? if($cate_id==0) { ?>class="current"<? } ?>><a href="<? echo reUrl('Code/hot?cate_id=0&type='.$type.'&cate_id2=0&p=1');; ?>">全部</a></dd>
                        <? if(is_array($cates)) { foreach($cates as $item) { ?>                        <? if($item['level']==0) { ?>
                        <dd <? if($cate_id==$item['id']) { ?>class="current"<? } ?>><a href="<? echo reUrl('Code/hot?cate_id='.$item['id'].'&type='.$type.'&cate_id2=0&p=1');; ?>"><?=$item['name']?></a></dd>
                        <? } ?>
                        <? } } ?>                    </dl>
                    <? if($cate_children) { ?>
                    <dl class="filter clear">
                    	<dt style="height: 40px;">二级类型：</dt>
                        <? if(is_array($cate_children)) { foreach($cate_children as $item) { ?>                        <dd <? if($cate_id2==$item['id']) { ?>class="current"<? } ?>><a href="<? echo reUrl('Code/hot?cate_id='.$cate_id.'&type='.$type.'&cate_id2='.$item['id'].'&p=1');; ?>"><?=$item['name']?></a></dd>
                        <? } } ?>                    </dl>
                    <? } ?>
                    <dl class="filter clear">
                        <dt>排序：</dt>
                        <dd <? if($type==1) { ?>class="current"<? } ?>><a href="<? echo reUrl('Code/hot?cate_id='.$cate_id.'&type=1&cate_id2='.$cate_id2.'&p=1');; ?>">昨日最热</a></dd><dd <? if($type==2) { ?>class="current"<? } ?>><a href="<? echo reUrl('Code/hot?cate_id='.$cate_id.'&type=2&cate_id2='.$cate_id2.'&p=1');; ?>">今日最热</a></dd><dd <? if($type==3) { ?>class="current"<? } ?>><a href="<? echo reUrl('Code/hot?cate_id='.$cate_id.'&type=3&cate_id2='.$cate_id2.'&p=1');; ?>">本周最热</a></dd><dd <? if($type==4) { ?>class="current"<? } ?>><a href="<? echo reUrl('Code/hot?cate_id='.$cate_id.'&type=4&cate_id2='.$cate_id2.'&p=1');; ?>">本月最热</a></dd>
                    </dl>
